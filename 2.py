...
class User:
    def __init__(self,name,email,user_id):
        ...
        def borrow_book(self, book):
            self.borrowed_books.append(book)  # Add book to the borrowed_books list
            print(f"{self.name} has borrowed '{book.title}'")

        def return_borrowed_book(self, book):
            if book in self.borrowed_books:
                self.borrowed_books.remove(book)  # Remove book from the borrowed_books list
    ...
    class Librarian(User):
        def __init__(self, name, email, user_id, library_books):
            ...
            self.library_books = library_books  # List to store library books

        def add_book_to_library(self, book):
            self.library_books.append(book)  # Add book to library's collection
            print(f"{self.name} added '{book.title}' to the library.")

        def remove_book_from_library(self, book):
            if book in self.library_books:
                self.library_books.remove(book)  # Remove book from library's collection
    ...
    class Library:
        def __init__(self):
            ...
            self.users = []  # List of users registered in the library

        def register_user(self, user):
            self.users.append(user)  # Add user to the library
        ...
        def list_available_books(self):
            ...
            for book in self.books:
                if book not in [borrowed_book for user in self.users for borrowed_book in user.borrowed_books]:
                    book.display_info()  # Display book info if it's available
    ...
    # List available books in the library after returning a book
    library.list_available_books()